...
function App() {
  ...
  // Auto-detect which section is visible and update toggle
  useEffect(() => {
    ...
    const observerCallback = (entries: IntersectionObserverEntry[]) => {
      entries.forEach((entry) => {
        if (entry.isIntersecting) {
          if (entry.target === nexaRef.current) {
            setIsHiring(true)
          } else if (entry.target === novaRef.current) {
            setIsHiring(false)
          }
        }
      })
    ...
    if (novaRef.current) {
      observer.observe(novaRef.current)
    }
    if (nexaRef.current) {
      observer.observe(nexaRef.current)
    }
  ...
  return (
    <div className="min-h-screen bg-linear-to-br from-[#EDE4FD] to-[#A78BFA]">
      ...
      <div style={{ height: navRef.current?.offsetHeight ?? 0 }} />
      ...
      <div className="py-6 sm:py-8 md:py-12 px-4 sm:px-6 md:px-12 lg:px-24 bg-[#e8eef8]">
        <div className="border border-gray-300 bg-white rounded-xl sm:rounded-2xl my-4 sm:my-6">
          ...
          <div className="mx-4 sm:mx-6 md:mx-8 mb-6 sm:mb-8">
            <VideoPlayer
              src="https://download-video-ak.vimeocdn.com/v3-1/playback/9071ea80-6cc6-417e-8641-c4c4f7896169/94a0d702?__token__=st=1770447976~exp=1770451576~acl=%2Fv3-1%2Fplayback%2F9071ea80-6cc6-417e-8641-c4c4f7896169%2F94a0d702%2A~hmac=8a45a22e3e6206a3c0b0380e9e09f697c8e5b0654e7ea33ea2418655d7d326c6&r=dXMtZWFzdDE%3D"
